...
#SAMPLE_RANGE_NAME = 'Sheet1!A1:O7000'
SAMPLE_RANGE_NAME = 'FiberMonitoringDataDump!A1:O7000'
...
#       all_data.append(ds)
   ...
   # df = pd.concat(all_data, axis=1)
    ...
    with open('blocks.csv') as csvfile:
        ...
        for row in readCSV:
            if iter == 0:
                ...
                df = df.append({colums[0]:row[0],colums[1]:row[1],colums[2]:row[2],colums[3]:row[3],colums[4]:row[4],colums[5]:row[5],colums[6]:row[6],colums[7]:row[7],colums[8]:row[8],colums[9]:row[9],colums[10]:row[10],colums[11]:row[11],colums[12]:row[12],colums[13]:row[13],colums[14]:row[14]},ignore_index=True)
    ...
    wks = sh.worksheet('title','FiberMonitoringDataDump')
