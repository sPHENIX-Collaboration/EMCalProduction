...
#SAMPLE_RANGE_NAME = 'Sheet1!A1:N7000'
SAMPLE_RANGE_NAME = 'FiberCountingDataDump!A1:N7000'
...
def main():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            creds = flow.run_local_server(port = 8049)
...
#       all_data.append(ds)
   ...
   # df = pd.concat(all_data, axis=1)
    ...
    with open('blocks.csv') as csvfile:
        ...
        for row in readCSV:
            if iter == 1:
                ...
                df = df.append({colums[0]:row[0],colums[1]:row[1],colums[2]:row[2],colums[3]:row[3],colums[4]:row[4],colums[5]:row[5],colums[6]:row[6],colums[7]:row[7],colums[8]:row[8],colums[9]:row[9],colums[10]:row[10],colums[11]:row[11],colums[12]:row[12],colums[13]:row[13]},ignore_index=True)
    ...
    wks = sh.worksheet('title','FiberCountingDataDump')
